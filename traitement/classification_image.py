...
for f in files:
    s = f.split("_")[3]
    s = s[:len(s)-4]
    cate = trouver_cat_product(
        int(s), 'data/X_train_update.csv', 'data/Y_train_CVw08PX.csv')
    print(int(s), cate)
    if not os.path.exists('imagesclassifiees/'+cate):
        os.makedirs('imagesclassifiees/'+cate)
    os.rename('images/image_train/'+f, 'imagesclassifiees/'+cate+'/'+f)
